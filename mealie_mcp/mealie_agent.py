...
def create_agent(
    ...
    for tag, (system_prompt, agent_name) in agent_defs.items():
        ...
        skill_dir_name = f"mealie-{tag.replace('_', '-')}"
...
def usage():
    print(
        ...
        "Examples:\n"
        "  [Simple]  mealie-agent \n"
        '  [Complex] mealie-agent --host "value" --port "value" --debug "value" --reload --provider "value" --model-id "value" --base-url "value" --api-key "value" --mcp-url "value" --mcp-config "value" --custom-skills-directory "value" --web\n'
    )
